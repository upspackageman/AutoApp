...
export class NhtsaService {
  baseUrl =
    'http://192.168.0.28:8000/_AutoApp/API/Controllers/NhtsaController.php';
  ...
  async getPdfData(url: any): Promise<Observable<any>> {
    ...
    return this.http
      .get<Blob>(this.baseUrl + '/getPdfFile/?link=' + url, {
  ...
  async getVideoPlayback(image: string): Promise<Observable<any>> {
    ...
    if (this.videoLocalStorageAvailable(key)) {
      const item = localStorage.getItem(key);
      return of(item);
    }

    return this.http
      .get<Blob>(this.baseUrl + '/videoPlayback/?image=' + image, {
        responseType: 'blob' as 'json',
      })
      .pipe(
        map((blob: Blob) => {
          ...
          reader.onloadend = () => {
            if (reader.result && typeof reader.result === 'string') {
              ...
              localStorage.setItem(key, reader.result); // Base64 encoded string of the Blob
            }
          };
  ...
  async getVehicleInvestigation(
    start: any,
    end: any,
    offset: any
  ): Promise<Observable<Investigation>> {
    return await this.http.get<Investigation>(
      this.baseUrl +
        '/getVehicleInvestigation?start=' +
        start +
        '&end=' +
        end +
        '&offset=' +
  ...
  getVehicleByYear(year: any): Observable<VehicleByYear> {
    ...
    return this.http
      .get<VehicleByYear>(this.baseUrl + '/getVehicleByYear?modelYear=' + year)
      .pipe(
        map((response) => {
  ...
  getVehicleModel(year: any, make: any) {
    ...
    return this.http
      .get<VehicleModel>(
        this.baseUrl + '/getVehicleModel?year=' + year + '&make=' + make
      )
      .pipe(
        map((response) => {
          this.nhtsaCache.set(key, response);
  ...
  getVehicleCrashRatingId(year: any, make: any, model: any) {
    ...
    return this.http
      .get<VehicleCrashRatingId>(
        this.baseUrl +
          '/getVehicleCrashRatingId?year=' +
          year +
          '&make=' +
          make +
          '&model=' +
  ...
  carSeatInspectionLocatorByZip(zip: any) {
    ...
    return this.http
      .get<CarInspectionZip>(
        this.baseUrl + '/carSeatInspectionLocatorByZip?zip=' + zip
      )
      .pipe(
        map((response) => {
          this.nhtsaCache.set(key, response);
  ...
  carSeatInspectionLocatorByState(state: any) {
    ...
    return this.http
      .get<CarInspectionState>(
        this.baseUrl + '/carSeatInspectionLocatorByState?state=' + state
      )
      .pipe(
        map((response) => {
          this.nhtsaCache.set(key, response);
  ...
  getVehicleCrashRating(id: any) {
    ...
    return this.http
      .get<VehicleCrashRating>(
        this.baseUrl + '/getVehicleCrashRating/?id=' + id
      )
      .pipe(
        map((response) => {
          this.nhtsaCache.set(key, response);
  ...
  videoLocalStorageAvailable(key: any): boolean {
    try {
      const videoList = localStorage.getItem(key);
